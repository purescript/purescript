...
compile :: FilePath -> P.Options -> [FilePath] -> Bool -> IO ()
compile outputDir opts input noPrefix' = do
  ...
  where prefix = case noPrefix' of
          True -> Nothing
          False -> Just("Generated by psc-make Version " ++ showVersion Paths.version  ++ "\n")
...
noPrefix = value $ flag $ (optInfo ["p", "no-prefix" ])
    ...
    where 
      p = "// Generated by psc-make Version " ++ showVersion Paths.version  ++ "\n"        


term :: FilePath -> Term (IO ())
term prelude = compile <$> outputDirectory <*> options <*> inputFilesAndPrelude prelude <*> noPrefix
