...
lintUnused (Module modSS _ mn modDecls exports) =
  ...
  lintDeclaration declToLint = do
    ...
    go (Var _ (Qualified Nothing v)) = (S.singleton $ IdentName v, mempty)
    ...
    goType = everythingOnTypes (<>) $ \case
      TypeConstructor _ (Qualified Nothing t) -> (S.singleton $ TyName t, mempty)
      _ -> mempty
    ...
    declIdents :: Declaration -> (S.Set (SourceSpan, Name), S.Set (SourceSpan, Name))
    declIdents (ValueDecl (ss,_) ident _ _ _) = (S.empty, S.singleton (ss, IdentName ident))
    declIdents (TypeSynonymDeclaration (ss,_) ty _ _) = (S.empty, S.singleton (ss, TyName ty))
