...
import Data.Graph
import Data.List (zipWith3, lookup)
...
guessModuleIdentifier filename = ModuleIdentifier (packFile $ takeFileName (takeDirectory filename)) <$> guessModuleType (takeFileName filename)
  where
...
data ModuleElement
  = Require JSStatement Text (Either Text ModuleIdentifier)
  | Member JSStatement Bool Text JSExpression [Key]
  | ExportsList [(ExportType, Text, JSExpression, [Key])]
  | Other JSStatement
  | Skip JSStatement
  deriving (Show)
...
printErrorMessage :: ErrorMessage -> [Text]
...
checkImportPath :: Text -> ModuleIdentifier -> S.Set Text -> Either Text ModuleIdentifier
...
withDeps (Module modulePath fn es) = Module modulePath fn (map expandDeps es)
  ...
  imports = mapMaybe toImport es
    where
    toImport :: ModuleElement -> Maybe (Text, ModuleIdentifier)
  ...
  boundNames = mapMaybe toBoundName es
    where
    toBoundName :: ModuleElement -> Maybe Text
  ...
  dependencies :: ModuleIdentifier -> JSExpression -> [(ModuleIdentifier, Text)]
  dependencies m = everything (++) (mkQ [] toReference)
    where
    toReference :: JSExpression -> [(ModuleIdentifier, Text)]
    toReference (JSMemberDot mn _ nm)
      | JSIdentifier _ mn' <- mn
      , JSIdentifier _ nm' <- nm
      , Just mid <- lookup (toS mn') imports
      = [(mid, toS nm')]
...
-- Other constructor.
toModule :: forall m. (MonadError ErrorMessage m) => S.Set Text -> ModuleIdentifier -> Maybe FilePath -> JSAST -> m Module
toModule mids mid filename top
  ...
  toModuleElement stmt
    ...
    where
      toExport :: JSObjectProperty -> m (ExportType, Text, JSExpression, [Key])
      ...
      exportType (JSMemberSquare f _ _ _)
        | JSIdentifier _ "$foreign" <- f
        = pure ForeignReexport
      exportType (JSIdentifier _ s) = pure (RegularExport $ toS s)
...
matchRequire mids mid stmt
  ...
  = Just (toS importName, importPath')
  | otherwise
  = Nothing
...
extractLabel (JSPropertyString _ nm) = Just $ textValue nm
extractLabel (JSPropertyIdent _ nm) = Just $ toS nm
...
codeGen :: Maybe Text -- ^ main module
        -> Text -- ^ namespace
        -> [Module] -- ^ input modules
        -> Maybe FilePath -- ^ output filename
        -> (Maybe SourceMapping, Text)
codeGen optionsMainModule optionsNamespace ms outFileOpt = (fmap sourceMapping outFileOpt, toS rendered)
  where
  rendered = renderToText (JSAstProgram (prelude : concatMap fst modulesJS ++ maybe [] runMain optionsMainModule) JSNoAnnot)

  sourceMapping :: FilePath -> SourceMapping
  ...
  moduleToJS (Module mn _ ds) = (wrap (moduleName mn) (indent (concat jsDecls)), lengths)
    ...
    declToJS (Require _ nm req) = withLength
      [
        JSVariable lfsp
          (cList [
            JSVarInitExpression (JSIdentifier sp $ toS nm)
    ...
    declToJS (ExportsList exps) = withLength $ map toExport exps
      ...
      toExport :: (ExportType, Text, JSExpression, [Key]) -> JSStatement
      toExport (_, nm, val, _) =
        JSAssignStatement
  ...
  prelude = JSVariable (JSAnnot tokenPosnEmpty [ CommentA tokenPosnEmpty $ "// Generated by purs bundle " ++ showVersion Paths.version
              ...
              (cList [
                JSVarInitExpression (JSIdentifier sp $ toS optionsNamespace)
  ...
  moduleReference :: JSAnnot -> Text -> JSExpression
  moduleReference a mn =
    JSMemberSquare (JSIdentifier a $ toS optionsNamespace) JSNoAnnot
      (str mn) JSNoAnnot

  str :: Text -> JSExpression
  str s = JSStringLiteral JSNoAnnot $ toS $ "\"" <> s <> "\""


  emptyObj :: JSAnnot -> JSExpression
  emptyObj a = JSObjectLiteral a (JSCTLNone JSLNil) JSNoAnnot

  wrap :: Text -> [JSStatement] -> [JSStatement]
  wrap mn ds =
    [
...
bundleSM :: (MonadError ErrorMessage m)
       => [(ModuleIdentifier, Maybe FilePath, Text)] -- ^ The input modules.  Each module should be javascript rendered from the compiler.
       -> [ModuleIdentifier] -- ^ Entry points.  These module identifiers are used as the roots for dead-code elimination
       -> Maybe Text -- ^ An optional main module.
       -> Text -- ^ The namespace (e.g. PS).
       -> Maybe FilePath -- ^ The output file name (if there is one - in which case generate source map)
       -> m (Maybe SourceMapping, Text)
bundleSM inputStrs entryPoints mainModule namespace outFilename = do
  ...
  input <- forM inputStrs $ \(ident, filename, js) -> do
                ast <- either (throwError . ErrorInModule ident . UnableToParseModule . toS) pure $ parse (toS js) (toS $ moduleName ident)
...
bundle :: (MonadError ErrorMessage m)
       => [(ModuleIdentifier, Text)] -- ^ The input modules.  Each module should be javascript rendered from the compiler.
       -> [ModuleIdentifier] -- ^ Entry points.  These module identifiers are used as the roots for dead-code elimination
       -> Maybe Text -- ^ An optional main module.
       -> Text -- ^ The namespace (e.g. PS).
       -> m Text
