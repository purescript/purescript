...
printErrorMessage (MissingEntryPoint mName) =
  [ "Couldn't find a module for the specified entry point: " ++ mName
  ]
...
bundle inputStrs entryPoints mainModule namespace = do
  forM_ entryPoints $ \mIdent@(ModuleIdentifier mname _) ->
    when (mIdent `notElem` map fst inputStrs) (throwError (MissingEntryPoint mname))
