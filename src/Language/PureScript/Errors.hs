...
prettyPrintSingleError full level showWiki e = flip evalState defaultUnknownMap $ do
  ...
  prettyPrintErrorMessage typeMap (ErrorMessage hints simple) =
    ...
    typeInformation | not (null types) = Just $ Box.hsep 1 Box.left [ line "where", paras types ]
      ...
      unknownInfo u = line $ markCode ("_" ++ show u) ++ " is an unknown type"
    ...
    renderSimpleErrorMessage (MultipleFixities name) =
      line $ "There are multiple fixity/precedence declarations for " ++ markCode (showIdent name)
    ...
    renderSimpleErrorMessage (OrphanFixityDeclaration op) =
      line $ "The fixity/precedence declaration for " ++ markCode (show op) ++ " should appear in the same module as its definition."
    ...
    renderSimpleErrorMessage (UnknownModule mn) =
      line $ "Unknown module " ++ markCode (runModuleName mn)
    renderSimpleErrorMessage (UnknownType name) =
      line $ "Unknown type " ++ markCode (showQualified runProperName name)
    renderSimpleErrorMessage (UnknownTypeOp name) =
      line $ "Unknown type operator " ++ markCode (showQualified showIdent name)
    renderSimpleErrorMessage (UnknownTypeClass name) =
      line $ "Unknown type class " ++ markCode (showQualified runProperName name)
    renderSimpleErrorMessage (UnknownValue name) =
      line $ "Unknown value " ++ markCode (showQualified showIdent name)
    renderSimpleErrorMessage (UnknownTypeConstructor name) =
      line $ "Unknown type constructor " ++ markCode (showQualified runProperName name)
    renderSimpleErrorMessage (UnknownDataConstructor dc tc) =
      line $ "Unknown data constructor " ++ markCode (showQualified runProperName dc) ++ foldMap ((" for type constructor " ++) . markCode . showQualified runProperName) tc
    renderSimpleErrorMessage (UnknownImportType mn name) =
      paras [ line $ "Cannot import type " ++ markCode (runProperName name) ++ " from module " ++ markCode (runModuleName mn)
            , line "It either does not exist or the module does not export it."
            ]
    renderSimpleErrorMessage (UnknownExportType name) =
      line $ "Cannot export unknown type " ++ markCode (runProperName name)
    renderSimpleErrorMessage (UnknownImportTypeOp mn name) =
      paras [ line $ "Cannot import type operator " ++ markCode (showIdent name) ++ " from module " ++ markCode (runModuleName mn)
            , line "It either does not exist or the module does not export it."
            ]
    renderSimpleErrorMessage (UnknownExportTypeOp name) =
      line $ "Cannot export unknown type operator " ++ markCode (showIdent name)
    renderSimpleErrorMessage (UnknownImportTypeClass mn name) =
      paras [ line $ "Cannot import type class " ++ markCode (runProperName name) ++ " from module " ++ markCode (runModuleName mn)
            , line "It either does not exist or the module does not export it."
            ]
    renderSimpleErrorMessage (UnknownExportTypeClass name) =
      line $ "Cannot export unknown type class " ++ markCode (runProperName name)
    renderSimpleErrorMessage (UnknownImportValue mn name) =
      paras [ line $ "Cannot import value " ++ markCode (showIdent name) ++ " from module " ++ markCode (runModuleName mn)
            , line "It either does not exist or the module does not export it."
            ]
    renderSimpleErrorMessage (UnknownExportValue name) =
      line $ "Cannot export unknown value " ++ markCode (showIdent name)
    renderSimpleErrorMessage (UnknownExportModule name) =
      paras [ line $ "Cannot export unknown module " ++ markCode (runModuleName name)
    ...
    renderSimpleErrorMessage (UnknownImportDataConstructor mn tcon dcon) =
      line $ "Module " ++ markCode (runModuleName mn) ++ " does not export data constructor " ++ markCode (runProperName dcon) ++ " for type " ++ markCode (runProperName tcon)
    ...
    renderSimpleErrorMessage (DuplicateTypeOpExport nm) =
      line $ "Duplicate export declaration for type operator " ++ markCode (showIdent nm)
    ...
    renderSimpleErrorMessage (ClassOperator className opName) =
      paras [ line $ "Type class " ++ markCode (runProperName className) ++ " declares operator " ++ markCode (showIdent opName) ++ "."
    ...
    renderSimpleErrorMessage (NotExhaustivePattern bs b) =
      paras [ line "A case expression could not be determined to cover all inputs."
            ...
            , line $ "Non-exhaustive patterns for values without a " ++ markCode "Partial" ++ " constraint will be disallowed in PureScript 0.9."
            ]
    ...
    renderSimpleErrorMessage (DeprecatedOperatorDecl name) =
      paras [ line $ "The operator " ++ markCode ("(" ++ name ++ ")") ++ " was declared as a value rather than an alias for a named function."
            , line "Operator aliases are declared by using a fixity declaration, for example:"
            , indent $ line $ "infixl 9 someFunction as " ++ markCode name
    ...
    renderSimpleErrorMessage (DeprecatedClassImport mn name) =
      paras [ line $ "Class import from " ++ markCode (runModuleName mn) ++ " uses deprecated syntax that omits the 'class' keyword:"
    ...
    renderSimpleErrorMessage (DeprecatedClassExport name) =
      paras [ line $ "Class export uses deprecated syntax that omits the " ++ markCode "class" ++ " keyword:"
            , indent $ line $ runProperName name
            , line "Should instead use the form:"
            , indent $ line $ "class " ++ runProperName name
            , line "The deprecated syntax will be removed in PureScript 0.9."
    ...
    renderSimpleErrorMessage (DuplicateImportRef ref) =
      line $ "Import list contains multiple references to " ++ markCode ref

    renderSimpleErrorMessage (DuplicateExportRef ref) =
      line $ "Export list contains multiple references to " ++ markCode ref
    ...
    renderSimpleErrorMessage (RedundantEmptyHidingImport mn) =
      line $ "The import for module " ++ markCode (runModuleName mn) ++ " is redundant as all members have been explicitly hidden."
    ...
    renderSimpleErrorMessage (InvalidOperatorInBinder op fn) =
      paras [ line $ "Operator " ++ markCode (showIdent op) ++ " cannot be used in a pattern as it is an alias for function " ++ markCode (showIdent fn) ++ "."
