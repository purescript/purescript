...
everythingOnValues (<>) f g h i j = (f', g', h', i', j')
  ...
  f' d@(ValueDeclaration vd) = foldl (<>) (f d) (fmap h' (valdeclBinders vd) ++ concatMap (\(GuardedExpr _ grd v) -> fmap k' grd ++ [g' v]) (valdeclExpression vd))
...
everythingWithContextOnValues s0 r0 (<>) f g h i j = (f'' s0, g'' s0, h'' s0, i'' s0, j'' s0)
  ...
  f' s (ValueDeclaration vd) = foldl (<>) r0 (fmap (h'' s) (valdeclBinders vd) ++ concatMap (\(GuardedExpr _ grd v) -> fmap (k' s) grd ++ [g'' s v]) (valdeclExpression vd))
