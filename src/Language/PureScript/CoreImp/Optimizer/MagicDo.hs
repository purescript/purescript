...
import Language.PureScript.CoreImp.AST ( everything, everywhere, everywhereTopDown, AST(Var, Unary, While, For, ForIn, IfElse, Function, Block, Return, ObjectLiteral, Assignment, Indexer, StringLiteral, VariableIntroduction, App), InitializerEffects(UnknownEffects), UnaryOperator(Not) )
...
magicDoEff = magicDo CLibs.M_Control_Monad_Eff CLibs.effDictionaries

magicDoEffect :: (AST -> AST) -> AST -> AST
magicDoEffect = magicDo CLibs.M_Effect CLibs.effectDictionaries

magicDoST :: (AST -> AST) -> AST -> AST
magicDoST = magicDo CLibs.M_Control_Monad_ST_Internal CLibs.stDictionaries

magicDo :: ModuleName -> CLibs.EffectDictionaries -> (AST -> AST) -> AST -> AST
magicDo effectModule CLibs.EffectDictionaries{..} expander = everywhereTopDown convert
  where
  ...
  isBind (expander -> App _ (Ref CLibs.P_bind) [Ref dict]) = (effectModule, edBindDict) == dict
  isBind _ = False
  -- Check if an expression represents a call to @discard@
  isDiscard (expander -> App _ (expander -> App _ (Ref CLibs.P_discard) [Ref CLibs.P_discardUnit]) [Ref dict]) = (effectModule, edBindDict) == dict
  isDiscard _ = False
  -- Check if an expression represents a monomorphic call to pure or return for the Eff applicative
  isPure (expander -> App _ (Ref CLibs.P_pure) [Ref dict]) = (effectModule, edApplicativeDict) == dict
  isPure _ = False
...
inlineST = everywhere convertBlock
  ...
  convertBlock (App s1 (Ref CLibs.P_run) [arg]) =
  ...
  -- Convert a block in a safe way, preserving object wrappers of references, -- or in a more aggressive way, turning wrappers into local variables depending on the
  -- agg(ressive) parameter.
  convert agg (App s1 (Ref CLibs.P_new) [arg]) =
   Function s1 Nothing [] (Block s1 [Return s1 $ if agg then arg else ObjectLiteral s1 [(mkString CLibs.stRefValue, arg)]])
  convert agg (App _ (App s1 (Ref CLibs.P_read) [ref]) []) =
    if agg then ref else Indexer s1 (StringLiteral s1 CLibs.stRefValue) ref
  convert agg (App _ (App _ (App s1 (Ref CLibs.P_write) [arg]) [ref]) []) =
    if agg then Assignment s1 ref arg else Assignment s1 (Indexer s1 (StringLiteral s1 CLibs.stRefValue) ref) arg
  convert agg (App _ (App _ (App s1 (Ref CLibs.P_modify) [func]) [ref]) []) =
    if agg then Assignment s1 ref (App s1 func [ref]) else Assignment s1 (Indexer s1 (StringLiteral s1 CLibs.stRefValue) ref) (App s1 func [Indexer s1 (StringLiteral s1 CLibs.stRefValue) ref])
  convert _ other = other
  -- Find all ST Refs initialized in this block
  findSTRefsIn = everything (++) isSTRef
    where
    isSTRef (VariableIntroduction _ ident (Just (_, App _ (App _ (Ref CLibs.P_new) [_]) []))) = [ident]
    isSTRef _ = []
  -- Find all STRefs used as arguments to read, write, modify
  findAllSTUsagesIn = everything (++) isSTUsage
    where
    isSTUsage (App _ (App _ (Ref CLibs.P_read) [ref]) []) = [ref]
    isSTUsage (App _ (App _ (App _ (Ref f) [_]) [ref]) []) | f `elem` [CLibs.P_write, CLibs.P_modify] = [ref]
    isSTUsage _ = []
