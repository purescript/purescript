...
data AST
  ...
  | Import (Maybe SourceSpan) Text (Maybe AST)
...
withSourceSpan withSpan = go where
  ...
  go (Import _ j1 j2) = Import ss j1 j2
  go (Export _ js) = Export ss js
  go (ExportStandard _ js) = ObjectLiteral ss js
  go (ExportForeign _ js) = ObjectLiteral ss js
  go (Var _ s) = Var ss s
...
getSourceSpan = go where
  ...
  go (Import ss _ _) = ss
  go (Export ss _) = ss
  go (ExportStandard ss _) = ss
  go (ExportForeign ss _) = ss
  go (Var ss _) = ss
  go (Block ss _) = ss
...
everywhere f = go where
  ...
  go (ExportStandard ss js) = f (ObjectLiteral ss (map (fmap go) js))
  go (ExportForeign ss js) = f (ObjectLiteral ss (map (fmap go) js))
...
everywhereTopDownM f = f >=> go where
  ...
  go (ExportStandard ss js) = ObjectLiteral ss <$> traverse (sndM f') js
  go (ExportForeign ss js) = ObjectLiteral ss <$> traverse (sndM f') js
