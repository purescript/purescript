...
moduleToCoreFn env (A.Module modSS coms mn decls (Just exps)) =
  ...
  exprToCoreFn :: SourceSpan -> [Comment] -> Maybe (Type SourceAnn) -> A.Expr -> Expr Ann
  ...
  getConstructorMeta ctor =
    ...
    numConstructors
      :: (Qualified (ProperName 'ConstructorName), (DataDeclType, ProperName 'TypeName, Type SourceAnn, [Ident]))
      -> Int
...
mkTypeClassConstructor :: SourceAnn -> [Constraint SourceAnn] -> [A.Declaration] -> Expr Ann
