...
deriveNewtypeInstance ss mn syns ndis className ds tys tyConNm dargs = do
    ...
    go tyCon
  where
    go (DataDeclaration _ Newtype _ tyArgNames [(DataCtorDeclarationData _ _ [(_, wrapped)])]) = do
...
deriveGenericRep ss mn syns ds tyConNm tyConArgs repTy = do
  ...
  where
    ...
    makeInst (DataCtorDeclarationData _ ctorName args) = do
...
deriveEq ss mn syns ds tyConNm = do
  ...
  where
    ...
    mkCtorClause :: DataCtorDeclarationData -> m CaseAlternative
    mkCtorClause (DataCtorDeclarationData _ ctorName tys) = do
...
deriveOrd ss mn syns ds tyConNm = do
  ...
  where
    ...
    mkCtorClauses :: (DataCtorDeclarationData, Bool) -> m [CaseAlternative]
    mkCtorClauses ((DataCtorDeclarationData _ ctorName tys), isLast) = do
      identsL <- replicateM (length tys) (freshIdent "l")
      identsR <- replicateM (length tys) (freshIdent "r")
      tys' <- mapM (replaceAllTypeSynonymsM syns . snd) tys
...
deriveNewtype ss mn syns ds tyConNm tyConArgs unwrappedTy = do
  ...
  where
    ...
    go (DataDeclaration (ss', _) Newtype name args dctors) = do
      ...
      let (DataCtorDeclarationData _ ctorName [(_, ty)]) = head dctors
...
deriveFunctor ss mn syns ds tyConNm = do
  ...
  where
    ...
    mkCtorClause :: Text -> Ident -> DataCtorDeclarationData -> m CaseAlternative
    mkCtorClause iTyName f (DataCtorDeclarationData _ ctorName ctorTys) = do
