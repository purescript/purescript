...
desugarDecl mn exps = go
  ...
  go (TypeInstanceDeclaration _ _ _ _ _ _ _ (DerivedInstance _)) = internalError "Derived instanced should have been desugared"
  ...
  go d@(TypeInstanceDeclaration sa _ _ name deps className tys (DerivedInstanceWithDictionary dict)) = do
