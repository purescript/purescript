...
collapseBindingGroups =
  ...
  go (BindingGroupDeclaration ds) =
    map (\(ident, nameKind, val) -> ValueDeclaration ident nameKind [] [([], val)]) ds
...
usedIdents moduleName = nub . usedIdents' S.empty . getValue
  ...
  getValue (ValueDeclaration _ _ [] [([], val)]) = val
...
toBindingGroup moduleName (CyclicSCC ds') =
  ...
  cycleError (ValueDeclaration n _ _ [([], _)]) = errorMessage $ CycleInDeclaration n
...
fromValueDecl (ValueDeclaration ident nameKind [] [([], val)]) = (ident, nameKind, val)
