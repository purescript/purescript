...
infer' v@(Constructor _ c) = do
  ...
  case M.lookup c (dataConstructors env) of
    Nothing -> throwError . errorMessage . UnknownName . Left . fmap DctorName $ c
...
infer' (UnknownValue name) = do
  ty <- freshType
  ctx <- getLocalContext
  tell . errorMessage . UnknownName . Right $ UnknownValueHint name ty ctx
...
inferBinder val (ConstructorBinder ss ctor binders) = do
  ...
  case M.lookup ctor (dataConstructors env) of
    ...
    _ -> throwError . errorMessage' ss . UnknownName . Left . fmap DctorName $ ctor
  where
