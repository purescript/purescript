...
overTypes f = let (_, f', _) = everywhereOnValues id g id in f'
  ...
  g (TypeClassDictionary con sco) = TypeClassDictionary (f con) sco
  g other = other
...
infer' (SuperClassDictionary con) = do
  dicts <- getTypeClassDictionaries
  return $ TypeClassDictionary con dicts
...
check' val t@(ConstrainedType constraints ty) = do
  ...
  newDictionaries :: [Integer] -> Qualified Ident -> Constraint -> m [TypeClassDictionaryInScope]
  newDictionaries path name con = do
    tcs <- gets (typeClasses . checkEnv)
    let (className, instanceTy) = stripTypeArguments [] con
      ...
      Just (args, _, superclasses) -> do
        supDicts <- join <$> zipWithM (\(supName, supArgs) index ->
                                          newDictionaries (index : path)
                                                          name
                                                          (foldl TypeApp (TypeConstructor supName) (instantiateSuperclass (map fst args) supArgs instanceTy))
                                      ) superclasses [0..]
...
check' (SuperClassDictionary con) _ = do
  {-
