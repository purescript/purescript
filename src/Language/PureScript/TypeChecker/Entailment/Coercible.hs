...
import Language.PureScript.Errors (DeclarationRef(..), ErrorMessageHint(..), ExportSource, ImportDeclarationType(..), MultipleErrors, SimpleErrorMessage(..), SourceAnn, errorMessage, UnknownsHint (NoUnknowns, Unknowns))
...
insoluble k a b =
  ...
  errorMessage $ NoInstanceFound (srcConstraint Prim.Coercible [k] [a, b] Nothing) [] (if any containsUnknowns [a, b] then Unknowns else NoUnknowns)
