...
checkEscapedSkolems moduleName ty = do
  ...
  let typesInScope = E.types env
      ...
      go ty' = \case
        Skolem ss name _ _ _ | M.notMember (Qualified (ByModuleName moduleName) (ProperName name)) typesInScope -> (ty', [(fst ss, name, ty')])
        _ -> (ty', [])
...
kindOfLocalTypeSynonym
  ...
  -> TypeDeclarationArgs
  -> m TypeDeclarationResult
kindOfLocalTypeSynonym moduleName typeDecl@((ss, _), synName, _, _) = do
  synKind <- existingSignatureOrFreshKind ByNullSourcePos ss synName
...
existingSignatureOrFreshKind
  ...
  -> ProperName 'TypeName
  -> m SourceType
existingSignatureOrFreshKind qb ss name = do
  env <- getEnv
  case M.lookup (Qualified qb name) (E.types env) of
...
kindsOfAll moduleName syns dats clss = withFreshSubstitution $ do
  synDict <- for syns $ \(sa, synName, _, _) -> (synName,) <$> existingSignatureOrFreshKind (ByModuleName moduleName) (fst sa) synName
  datDict <- for dats $ \(sa, datName, _, _) -> (datName,) <$> existingSignatureOrFreshKind (ByModuleName moduleName) (fst sa) datName
  clsDict <- for clss $ \(sa, clsName, _, _, _) -> fmap (coerceProperName clsName,) $ existingSignatureOrFreshKind (ByModuleName moduleName) (fst sa) $ coerceProperName clsName
