...
coercible = primClassOf (P.primSubName "Coerce") "Coercible" $ T.unlines
  ...
  , ""
  , "* Two types of kind `Type` may be coercible, but only if their heads are"
  , "the same. For example, `Coercible (Maybe a) (Either a b)` does not hold"
  , "because `Maybe` and `Either` are different. Those types don't share a common"
  , "runtime representation so coercing between them would be unsafe. In addition"
  , "their arguments may need to be identical or coercible, depending on the"
  , "_roles_ of the head's type parameters. Roles are documented in"
  , "[the PureScript language reference](https://github.com/purescript/documentation/blob/master/language/Roles.md)."
  , ""
