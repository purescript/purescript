...
coercible = primClassOf (P.primSubName "Coerce") "Coercible" $ T.unlines
  ...
  , ""
  , "* Terms of different types are not coercible:"
  , ""
  , "      data Maybe a = Nothing | Just a"
  , "      data Either a b = Left a | Right b"
  , ""
  , "`Coercible (Maybe b) (Either a b)` does not hold. Those types don't share a"
  , "common runtime representation so coercing between them would be unsafe."
  , ""
  , "* Terms of the same types (`data` or `newtype`) may be coercible, depending"
  , "on the _roles_ of their parameters. Roles are documented in [the PureScript"
