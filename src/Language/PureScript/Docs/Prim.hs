...
record = primType "Record" $ T.unlines
  ...
  , ""
  , "Because `Record` uses `Row`s, one can define an open or closed record."
  , ""
  , "    type ClosedRecord = { name :: String }"
  , "    type OpenRecord allOtherRows = { name :: String | allOtherRows }"
  , "    type FinalRecord = OpenRecord ( anotherLabel :: Boolean )"
  , "    --   FinalRecord = { name :: String, anotherLabel :: Boolean }"
  , ""
