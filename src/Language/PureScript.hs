...
compile' env opts ms ver = do
  ...
  let pjs = addPrefix (optionsGeneratedBy opts) ver $ prettyPrintJS js'
  return (pjs, exts, env')
  where
  mainModuleIdent = moduleNameFromString <$> optionsMain opts

addPrefix :: Bool -> String -> String -> String
addPrefix generate ver js = if generate then prefix ++ js else js
  where 
    prefix = "//Generated by PureScript version " ++ ver ++ "\n"
...
make outputDir opts ms version = do
  ...
  go env ((True, m@(Module moduleName' _ exps)) : ms') = do
    ...
    let js = addPrefix (optionsGeneratedBy opts) version pjs
    let exts = addPrefix (optionsGeneratedBy opts) version $ moduleToPs renamed env'
