...
compile opts input output externs = do
  ...
  case modules of
    Left err -> do
      ...
      exitFailure
    Right ms -> do
      case P.compile opts (map snd ms) ver of
        Left err -> do
...
generatedBy :: Term Bool
generatedBy = value $ flag $ (optInfo [ "g", "generated-by" ])
     { optDoc = "prefix output with a 'Generated by PureScript version X.Y.Z' comment" }

options :: Term P.Options
options = P.Options <$> noPrelude <*> noTco <*> performRuntimeTypeChecks <*> noMagicDo <*> runMain <*> noOpts <*> (Just <$> browserNamespace) <*> dceModules <*> codeGenModules <*> verboseErrors <*> generatedBy
