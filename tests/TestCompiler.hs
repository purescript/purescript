...
spec = do
  ...
  (supportExterns, passingTestCases, warningTestCases, failingTestCases) <- runIO $ do
    ...
    case supportExterns of
      Left errs -> fail (P.prettyPrintMultipleErrors True False errs)
...
assertCompiles supportExterns inputFiles =
  assert supportExterns inputFiles checkMain $ \e ->
    case e of
      Left errs -> return . Just . P.prettyPrintMultipleErrors True False $ errs
      Right _ -> do
...
assertCompilesWithWarnings supportExterns inputFiles shouldWarnWith =
  assert supportExterns inputFiles checkMain $ \e ->
    case e of
      Left errs ->
        return . Just . P.prettyPrintMultipleErrors True False $ errs
      Right warnings ->
        return
  ...
  printAllWarnings warnings =
    (<> "\n\n" <> P.prettyPrintMultipleErrors True False warnings)
