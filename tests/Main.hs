...
makeActions foreigns = (P.buildMakeActions modulesDir getFilePaths) 
  ...
  getFilePaths :: P.ModuleName -> (unused, Maybe (FilePath, P.ForeignJS))
  getFilePaths mn = (error "makeActions: fst read in getFilePaths", M.lookup mn foreigns)
...
runTest = fmap (either Left (Right . fst)) . P.runMake P.defaultOptions
...
compile inputFiles foreigns = runTest $ do
  ...
  P.make (makeActions foreigns) (map snd ms) False
