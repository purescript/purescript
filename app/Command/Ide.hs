...
command = Opts.helper <*> subcommands where
  ...
  server opts'@(ServerOptions dir globs outputPath port logLevel editorMode noWatch polling) = do
    ...
    -- TODO(Christoph): Remove these flags in the next breaking release (0.15.0?)
    when editorMode
    ...
    when polling
      (putText "The --polling flag is deprecated and ignored. purs ide no longer uses a file system watcher, instead it relies on its clients to notify it about updates")

    when noWatch
      (putText "The --no-watch flag is deprecated and ignored. purs ide no longer uses a file system watcher, instead it relies on its clients to notify it about updates")
