...
-- Due to not deprecating syntax yet, we have to handle a lot of possible cases.
-- We only warn when a `constraints` is included without a corresponding `forall`:
--
--                 ||    instance number    |
--                 ||=======================|
--                 || 1 | 2 | 3 | 4 | 5 | 6 |
-- generated name  || x | x | x |   |   |   |
-- has constraints || x | x |   | x | x |   |
-- has forall      || x |   |   | x |   |   |
